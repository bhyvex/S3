...
class DpClient {
    ...
    put(stream, size, params, reqUids, callback) {
        ...
        // store a fragment
        const streamFrag = (frag, fragIdx, cb) => {
        ...
        const streamData = (buf, len, startIdx, cb) => {
        ...
        stream.on('data', chunk => {
            ...
            if (dataLen >= fragSize) {
                ...
                streamData(buf, len, startIdx, (err, keysNb) => {
                    if (err) {
                        noError = false;
                        log.error('error from datastore',
                            { error: err, implName });
        ...
        stream.on('end', () => {
            if (!noError) {
                return cleanFrags('error from datastore', callback);
            }
            noError = true;
            let donesAll = 0;
            // last data fragments
            if (dataLen > 0 || fragCursor < this.ecParams.k) {
                ...
                streamData(buf, alignedSize - start,
