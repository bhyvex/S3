...
// default params for repair objects
const repairParams = {
...
class DpClient {
    ...
    put(stream, size, params, reqUids, callback) {
        ...
        function streamFrag(frag, fragIdx, cb) {
            ...
            log.debug('Streaming fragment', { fragID: fragIdx });
            client.put(stream, fullFragSize, params, reqUids, err => {
                if (err) {
                    log.error('error from datastore',
                        { error: err, implName });
                    return cb(err);
                }
                log.debug('Fragment is stored', { fragID: fragIdx });
                return cb();
            }, keys[fragIdx]);
        }
        ...
        function cleanFrags(error, cb) {
            ...
            return this.delete(storedKeys, reqUids, err => {
                if (err) {
                    log.debug('Failed to clean put', { error: err });
                }
                return cb(error);
            });
        }
        ...
        function streamData(buf, len, startIdx, cb) {
            ...
            while (cursor < len) {
                ...
                log.debug('Added header for fragment', { fragID: fragIdx });
        ...
        function done(key, cb) {
            ...
            const job = new CronJob({
                ...
                onTick: () => {
                    this.repair(key, err => {
                        if (err) {
                            process.stderr.write(`repair ${key}: ${err}\n`);
                        }
                    });
                },
                start: true,
        ...
        stream.on('data', chunk => {
            ...
            if (dataLen >= fragSize) {
                ...
                streamData.bind(this)(buf, len, startIdx, (err, keysNb) => {
                    ...
                    donesNb += keysNb;
                    if (donesNb === codeLen) {
                        return done.bind(this)(key, callback);
                    }
                    return undefined;
                });
            }
    ...
    repair(key, callback) {
        ...
        function repairFrags(lostFragsIds, liveFrags, cb) {
        ...
        keys.forEach((keyFrag, fragIdx) => {
            this.checkAvailFrag.bind(this)(keyFrag, log, (err, val) => {
                if (err) {
                ...
                if (lostFragsIds.length + fragments.length === codeLen) {
                    if (lostFragsIds.length > 0) {
                        return repairFrags.bind(this)(lostFragsIds, fragments,
                            callback);
                    }
                    return callback();
                }
    ...
    checkAvailFrag(key, log, callback) {
        ...
        function getFrag(key, reqUids, retriesNb, cb) {
        ...
        return getFrag.bind(this)(key, key, 0, callback);
    }
}
